...
- 运行模式。
- 项目识别关键词。
- 平台适配器要求的目标场次、目标票档、目标价格、数量和观演人配置提示。
- 最大提交次数、运行时长等安全上限。

平台特有字段由适配器提供定义。未选择必填项时，“开始”不可用。

只要运行模式包含回流阶段，目标场次、目标票档和目标价格就是必填项。它们在任务开始时生成不可变的目标快照，运行过程中不得根据页面当前选中项覆盖。
...
3. 启动前台服务和悬浮控件。
4. 将配置页面移到后台。
5. 进入 `WAIT_TARGET_APP`，等待用户自行打开售票应用。
...
| `event_keyword` | String | 项目校验关键词 |
| `target_session` | String? | 固定目标场次 |
| `target_tier` | String? | 固定目标票档名称 |
| `target_price_fen` | Long? | 固定目标单价，单位分 |
| `ticket_count` | Int? | 票数 |
| `adapter_config` | String | 平台特有配置 JSON |
| `max_submit_attempts` | Int | 最大提交次数 |
| `max_runtime_seconds` | Long | 最大运行时长 |
| `state` | String | 任务状态 |
...
| 字段 | 说明 |
| --- | --- |
| `targetTier` | 从任务配置读取的固定目标票档，运行期间只读 |
| `selectedTier` | 当前页面实际选中的票档；Android 重进后可为空 |
| `returnCycleState` | Android 回流循环阶段：检查票档、退出详情、重新进入 |
| `returnCycleCount` | 已完成的退出重进次数 |

`selectedTier` 只能记录页面临时状态，禁止写回或替换 `targetTier`。首次开售切换到回流阶段、暂停后恢复或页面重新进入时，都必须重新从任务快照取得 `targetTier`。
